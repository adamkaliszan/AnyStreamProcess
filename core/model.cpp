...
{
    auto iter = std::begin(agenda);
    while (iter != std::end(agenda)) {
        delete *iter;
        iter++;
    }
    agenda.clear();
...
void ModelTrClass::SimulatorSingleServiceSystem::stabilize(int noOfEvents)
{
    for( ;noOfEvents>0; noOfEvents--)
    {
        ...
        auto first = std::begin(agenda);

        SimulatorProcess *proc = *first;
        agenda.erase(first);
        ...
        if (noOfEvents % (1<<20) == 0) {
            for (auto iter = std::begin(agenda); iter != std::end(agenda); iter++) {
...
void ModelTrClass::SimulatorSingleServiceSystem::endCallService(ModelTrClass::SimulatorProcess *call)
{
    ...
    if (callPartialyServiced)
    {
        ...
        auto iter = agenda.find(callPartialyServiced);
        agenda.erase(iter);
        n_inServer +=t;
        n_inServer -= resPartOccupied;
        resPartOccupied = 0;
